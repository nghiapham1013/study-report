# [PM beginner] Những khái niệm và phương pháp cơ bản (P3 - Time management)
...
![](https://github.com/nghiapham1013/study-report/blob/201907-study-report/Time7.PNG)
...
![](https://github.com/nghiapham1013/study-report/blob/201907-study-report/Time2.PNG)
...
![](https://github.com/nghiapham1013/study-report/blob/201907-study-report/Time3.png)
...
Là việc ước tính loại và lượng tài nguyên cần thiết cho các activities  
Các tài nguyên bao gồm con người và các công cụ. Trong ngành IT thì con người chính là nguồn tài nguyên chủ yếu và đặc biệt quan trọng  
Đối với mỗi công việc thì đều có các cách thực hiện khác nhau nên trong process này người ta cũng sẽ đánh giá phương pháp làm việc nào là phù hợp nhất

Các resource được estimate sau cùng sẽ là những điều kiện cần có của các activities

##### 3.2.4 Estimate activity durations
Ở giai đoạn này chúng ta sẽ dựa trên những resource đã được lên danh sách ở `Estimate activity resource` process để ước lượng khoảng thời gian cần thiết để hoàn thiện công việc  
Thông thường người ta sẽ dùng 3 phương pháp để estimate thời gian hoàn thiện công việc: `Analogous estimation`, `Parametric estimation`, và `Three-point estimation`  
Chúng ta sẽ estimate thời gian cho từng activitiy và sau đó sẽ đưa ra con số estimate tổng

*Analogue estimation*  
Estimate dựa trên các kết quả thu được trong quá khứ. Time và cost có thể giảm đi nhưng các điều kiện, tình hình đặc biệt của dự án sẽ không được tính vào trong này. Kỹ năng của PIC cũng sẽ ảnh hưởng đến con số estimate ở đây
  
*Coefficient estimation*  
Là phương pháp estimate dựa trên những số liệu cũ và các con số khác  
Chúng ta sẽ dựa trên những dự án trong quá khứ và đặt ra các unit cost hoặc duration. Sau đó sẽ đánh giá xem activity trong dự án hiện tại cần bao nhiêu unit như vậy  
Độ chính xác của cách tính này sẽ tùy thuộc vào mô hình mà chúng ta lựa chọn
  
*Three-point estimation*  
Là phương pháp estimate theo giá trị trung bình của 3 trường hợp: tốt nhất có thể, trung bình, và tệ nhất có thể  
Công thức sẽ là:  `best_case` + `most_likely` * 4 + `worst_case` : 6

*Create schedule*  
Chúng ta sẽ dựa trên kết quả của các process từ trước đến giờ, thông qua ngày bắt đầu và kết thúc của activities để có thể tạo ra được schedule. Phương pháp được sử dụng để tạo schedule chính là `Critical path`  
Thông qua việc tạo schedule thì có thể so sánh lại với ngày kết thúc dự án dự kiến ban đầu, qua đó có thể rút ngắn thời gian dự án  
Kết quả của việc tạo schedule chính là chúng ta sẽ có được Milestones chart hoặc Bar chart

*Critical path method*  
Là phương pháp xác định thứ tự thực hiện activities mà không quan tâm đến những hạn chế về mặt resources  
Chúng ta sẽ xác định ngày bắt đầu sớm/muộn nhất cũng như ngày kết thúc sớm/muộn nhất của các activities
...
*Bar chart*  
Thể hiện ngày bắt đầu, kết thúc, quãng thời gian thực hiện của các activities bằng các thanh bar

![](https://github.com/nghiapham1013/study-report/blob/201907-study-report/Time8.png)

*Control schedule*  
Là phương pháp theo dõi trạng thái để có thể nắm được tiến độ, tình hình dự án cũng như kiểm soát được các thay đổi trong dự án  
Chúng ta cần luôn chủ động nắm bắt và thực hiện điều chỉnh theo những thay đổi phát sinh trong dự án  
Dựa trên những phương pháp như performance review hay schedule reduction thì chúng ta có thể đưa ra được kết quả đánh giá performance của các đầu việc

*Performance review*  
Đánh giá năng suất và so sánh giữa thực tế so với trong kế hoạch khác nhau như thế nào  
Phương pháp đánh giá được sử dụng ở đây là `Earned Value Management (EVM)`

*Earned Value Management (EVM)*  
Là phương pháp đánh giá kế hoạch cũng như năng suất của dự án

![](https://github.com/nghiapham1013/study-report/blob/201907-study-report/Time6.png)

*Schedule reduction*  
Chúng ta có thể rút ngắn schedule thông qua `Crashing` và `Fast tracking`  
Crashing: sử dụng tăng cường chi phí để sử dụng thêm nguồn lực, thêm thời gian và cũng chỉ làm việc nếu phần tăng cường tài nguyên trong thời gian ngắn  
...
*Work performance measurement results*  
Định lượng và đưa vào tài liệu những thông số về: cost variance (CV), schedule variance (SV), schedule efficiency index (SPI), cost efficiency index (CPI)  
Hiệu suất sẽ được đánh giá thông qua việc so sánh các con số thu được với các con số trong plan

### 3.3 Summary
Mục đích của Time management trong dự án là có thể giúp hoàn thành dự án đúng hạn. Hãy vận dụng Time management theo từng process và tạo ra được những kết quả cần thiết  
Vì PMBOK được thiết kế cho những dự án lớn nên hãy biết chắt lọc cũng như tối ưu hóa các process sao cho phù hợp với dự án của mình

Như vậy thông qua bài viết này chúng ta đã có những kiến thức sơ lược, những công việc cần làm và những output cần có khi vận dụng Time management vào dự án  
Tuy nhiên hãy đừng chỉ dừng lại trên lý thuyết. Hãy vận dụng những kiến thức sơ lược, bắt tay vào làm những công việc cần làm, và tạo ra được những output thực tế để có thể làm chủ được Time management
